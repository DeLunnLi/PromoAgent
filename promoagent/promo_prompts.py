...
PROMO_JSON_SCHEMA = """{
  ...
  "campaignGoal": "本次内容的转化目标，如 awareness|click|signup|feedback|launch_support|release_update",
  "targetUsers": ["目标用户"],
  "strongestAngles": ["最适合推广的切入点"],
  "promotionStrategy": {
    "coreAngle": "基于来源证据的主推广角度",
    "taskMode": "source_to_campaign|release_announcement|product_launch|ad_variant_test|content_repurpose|recommendation",
    "audienceSegments": [{"segment": "人群", "pain": "痛点/欲望", "evidence": "来源依据", "bestChannels": ["平台"]}],
    "channelFit": [{"platform": "平台", "why": "为什么适合", "risk": "不适合或需谨慎之处"}],
    "contentGraph": [{"node": "problem|evidence|action", "claim": "内容", "source": "来源"}],
    "creativeVariants": [{"platform": "平台", "hook": "开头/标题方向", "hypothesis": "为什么这个角度可能有效", "cta": "行动号召"}],
    "assetBriefs": [{"platform": "平台", "visual": "配图/截图/封面建议", "copySafeText": "封面文字建议"}],
    "qualityRubric": {
    ...
    "reviewGate": {
      "mustVerify": ["发布前必须核实的事实"],
      "missingContext": ["缺失但会显著提升转化的信息"],
      "doNotClaim": ["不能声称的内容"]
    }
  },
  "promotions": {
...
def build_promo_system_prompt() -> str:
    return "\n".join([
        ...
        "- 这是什么类型的推荐任务？软件工具、本地生活/餐饮、消费品、课程服务、活动、论文/文档的推荐理由完全不同。",
        "- 这是哪一种相似产品任务？URL/README 转多平台内容、版本发布公告、Product Hunt 上线、广告变体测试、还是长内容 repurpose？",
        "",
        "## 推荐任务适配",
        "- 软件/工具：写清楚使用场景、节省的步骤、适合谁，不要编造性能或用户规模。",
        "- 本地生活/餐饮：突出具体体验、位置/价格等证据、适合的场景，不要编造口味和评价。",
        "- 消费品：强调真实卖点、使用场景、购买顾虑和限制，不要虚构折扣或功效。",
        "- 课程/服务：说明问题、过程、适合人群和边界，不要承诺保证结果。",
        "- 活动：说明为什么值得去、适合谁、时间/地点/报名信息，不要虚构嘉宾或名额。",
        "- 论文/文档：解释研究问题、方法和读者收获，不要夸大结论。",
        "",
        "## 相似任务模式",
        "- Source → Campaign：像社媒 AI 工具一样，把一个 URL/README/文件拆成多个平台的原生内容，但要保持同一主张和品牌语气。",
        "- Release Announcement：如果来源像 README/CHANGELOG/release notes，要把代码变化翻译成用户价值，不要只罗列 commits。",
        "- Product Launch：如果适合 Product Hunt/Show HN，要生成清楚 tagline、maker story、首条评论/帖子方向和后续互动建议。",
        "- Ad Variant Test：为广告或推广图准备 2-4 个 hook 变体，说明每个变体测试的假设、目标人群和 CTA。",
        "- Content Repurpose：把长文/论文/PDF 拆成短帖、长帖、问答、评论区补充，而不是跨平台复制同一段。",
        "",
        "## 核心原则",
        "- 只使用来源证据中可核实的事实。无法验证的内容写 caveat，不要补编。",
        "- 如果用户没有给品牌语气、转化目标或目标人群，请做最保守的推断，并写入 reviewGate.missingContext。",
        "- 根据内容特点和受众，自主判断哪些平台最适合，选择 2-5 个平台生成内容。",
        "- 每个平台的内容要有明显差异：不只是改改措辞，结构和切入角度都要不同。",
        "- 输出 creativeVariants，用于 A/B 测试不同 hook，不要只给一个版本。",
        "- 输出 channelFit，解释为什么选择或避开某个平台。",
...
def build_promo_user_prompt(
    ...
    *,
    ...
    parts.extend([
        ...
        "",
        "**Step 0 — 判断任务模式和转化目标**",
        "先判断 taskMode：source_to_campaign / release_announcement / product_launch / ad_variant_test / content_repurpose / recommendation。",
        "再确定 campaignGoal：awareness / click / signup / feedback / launch_support / release_update。",
        "如果来源是 GitHub README、CHANGELOG、release notes 或 commit/release 信息，优先考虑 release_announcement 或 product_launch。",
        "",
        "**Step 1 — 挖掘真正的推广角度**",
        ...
        "",
        "**Step 2 — 选择受众、平台和内容图谱**",
        "输出 audienceSegments 和 channelFit：每个平台都要说明为什么适合，以及不适合/需谨慎之处。",
        "用证据中的具体材料（命令/价格/地址/图片/数据/评价）填充 contentGraph，",
        "标注每个 claim 来自哪里，确保每个平台都有可引用的原始证据。",
        "",
        "**Step 3 — 设计 hook 变体和素材建议**",
        "至少输出 2-4 个 creativeVariants，用不同 hook 测试不同人群或动机：问题痛点、结果收益、反差细节、幕后故事、社会证明。",
        "输出 assetBriefs：每个平台建议什么截图、封面图、演示 GIF、产品图或评论区补充材料。",
        "",
        "**Step 4 — 按平台写，真正有差异**",
        "不同平台的差异不只是语气——核心切入角度也应该不同：",
        "- 同一家餐厅：小红书切「隐藏宝藏发现感」，知乎切「性价比分析」，微信切「这个周末去哪吃」",
        "- 同一个工具：Show HN 切「解决了什么技术痛点」，小红书切「这个命令帮我省了多少时间」",
        "- 同一次版本发布：Twitter/X 切「刚发布」，LinkedIn 切「团队/用户价值」，GitHub release 切「变更摘要」，Product Hunt 切「为什么现在值得试」",
        "",
        "**Step 5 — 三轴审核和发布门禁**",
        "- Fidelity：每个 claim 都能在来源证据中找到依据？",
        "- Engagement：第一句话是否具体？读者能否在 3 秒内判断「这和我有关」？",
        "- Alignment：语气、节奏、格式是否真的像这个平台的原生内容？",
        "同时填写 reviewGate.mustVerify / missingContext / doNotClaim。",
        "",
        platform_hint,
        "",
        "JSON 输出结构：",
